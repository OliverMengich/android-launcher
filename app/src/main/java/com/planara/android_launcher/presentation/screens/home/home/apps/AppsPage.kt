            ...
            items(items=filteredApps) { ap->
                AppItem(
                    ...
                    onHideApp = {
                        textField=""
                        scope.launch {
                            viewModel.hideUnhideAppFc(app=ap, hidden = !ap.isHidden)
                        }
                    },
                    onUninstallApp = {
                    ...
                    onBlockApp = {
                        scope.launch {
                            val isBlocked = context.dataStore.data.first().blockedApps.find { it.packageName==ap.packageName }
                            if (isBlocked!=null){
                                Toast.makeText(context,"${ap.name} is already blocked.",Toast.LENGTH_SHORT).show()
                            }else{
