...
import com.planara.android_launcher.presentation.screens.home.home.calendar.NewEventPageEvent
import com.planara.android_launcher.presentation.screens.home.home.calendar.NewEventPageState
...
import com.planara.android_launcher.utils.notifyOptions
...
//            }
            ...
            Row(Modifier.fillMaxWidth().padding(all=16.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(
                    text= longToMilliSeconds(selectedEvent.start).formatTimeToRequiredFormat(tmFmt),
                    Modifier.clickable(enabled = isEditable,onClick={
                        category="startTime"
                        val tm = longToMilliSeconds(timestamp = selectedEvent.start)
                        timePickerState.hour = tm.hour
                        timePickerState.minute = tm.minute
                    }),
                    color = textColor,
                )
            }
            Row(
                Modifier.fillMaxWidth().padding(all=16.dp).clickable(enabled = isEditable,onClick={
                    category="endTime"
                    val tm = longToMilliSeconds(timestamp = selectedEvent.end)
                    timePickerState.hour = tm.hour
                    timePickerState.minute = tm.minute
                }),
                ...
                Text(
                    text= longToMilliSeconds(timestamp = selectedEvent.end).formatTimeToRequiredFormat(tmFmt),
                    Modifier.clickable{
                        category="endTime"
                        timePickerState.hour = longToMilliSeconds(timestamp = selectedEvent.end).hour
                        timePickerState.minute = longToMilliSeconds(timestamp = selectedEvent.end).minute
                    },
                    color = textColor,
                )
            }
            ...
            Column(Modifier.padding(all=16.dp)){
                Row(Modifier.fillMaxWidth()){
                    ...
                    Column(Modifier.weight(1f)) {
                        Text(
                            text="Notify ${selectedEvent.minutesBefore} minutes before",
                            color = textColor
                        )
